"""
Trains a multi-speaker VITS model on the OpenSLR Sinhala corpus.

Expects, relative to the project root:
  data/openslr_metadata.txt  -> produced by data_preparation/prepare_openslr_dataset.py
  data/wavs/*.wav             -> the audio referenced by openslr_metadata.txt

Usage (from the project root, e.g. in Colab after `%cd` into it):
  python model_training/train_openslr.py
"""
...
from formatters import openslr_formatter
from vocab_utils import build_characters_config

current_dir = os.path.dirname(os.path.abspath(__file__))
project_root = os.path.dirname(current_dir)
data_path = os.path.join(project_root, "data")
output_path = os.path.join(current_dir, "runs", "openslr")

METADATA_FILE = "openslr_metadata.txt"

dataset_config = BaseDatasetConfig(
    formatter="openslr_formatter",
    dataset_name="voicelk_openslr",
